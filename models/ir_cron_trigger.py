...
class IrCronTrigger(models.Model):
    ...
    def _check_image_cron_is_not_already_triggered(self):
        """ Ensure that there is a maximum of one trigger at a time for `ir_cron_send_allegro_offer`.
        ...
        :raise ValidationError: If the maximum number of coexisting triggers for
                                `ir_cron_send_allegro_offer` is reached
        """
        ir_cron_send_allegro_offer = self.env.ref(
            'odoo_allegro.ir_cron_send_allegro_offer', raise_if_not_found=False
        )

        if ir_cron_send_allegro_offer and self.cron_id.id != ir_cron_send_allegro_offer.id:
            return
